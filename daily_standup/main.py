...
class UltraFastSessionManagerWithSilenceHandling:
    ...
    async def generate_dynamic_silence_response(self, session_data: SessionData, silence_context: dict = None):
        ...
        try:
            # Get completed silence count for this session
            completed_count = getattr(session_data, "silence_response_count", 0)
            # Use *next* count only for context; don't mutate here
            silence_count = completed_count + 1
            ...
            # 🆕 Stage-aware fallback block starts here
            phase = (silence_context or {}).get("phase") or session_data.current_stage.value
            fallbacks_greeting = [
                f"{session_data.student_name}, no rush. I’m here when you’re ready.",
                f"All good, {session_data.student_name}. Take a moment, then we’ll continue.",
                f"I’ll wait for you, {session_data.student_name}.",
                f"We can start whenever you’re ready, {session_data.student_name}.",
            ]
            fallbacks_technical = [
                "Shall I simplify it or move to the next one?",
                "Would you like me to repeat the question?",
                "Do you want a hint, or shall I rephrase the question?",
                "Take your time—we can continue whenever you’re ready.",
            ]

            # pick pool based on phase
            pool = fallbacks_greeting if str(phase).lower() == "greeting" else fallbacks_technical
            last_line = getattr(session_data, "last_silence_line", None)
            choices = [x for x in pool if x != last_line] or pool
            session_data.last_silence_line = random.choice(choices)
            # 🆕 Stage-aware fallback block ends here
            ...
            if response_text == getattr(session_data, "last_silence_text", ""):
                logger.info("🔁 Prevented duplicate silence prompt")
                return  # or generate new one
            session_data.last_silence_text = response_text
    ...
    async def create_session_fast(self, websocket: Optional[Any] = None) -> SessionData:
        ...
        try:
            ...
            session_data.max_silence_responses = getattr(config, "MAX_SILENCE_RESPONSES", 5)  # ✅ Unlimited silence responses
            ...
            session_data.last_silence_response_ts = 0  # ✅ Initialize cooldown timestamp
            
            # ---- NEW: TTS kill/interrupt flags (from killedsilence) ----
            session_data.cancel_silence_tts = False           # Hard cancel flag for silence TTS
            session_data.is_user_speaking_live = False        # Live user speaking detection
            session_data.last_user_speech_ts = 0.0            # Timestamp of last user speech
            session_data.last_status = None                   # Last VAD status received
            session_data.silence_tts_active = False           # True while silence TTS streaming
            session_data.normal_tts_active = False            # True while normal AI TTS streaming
            session_data.silence_followup_running = False     # Follow-up loop flag
            session_data.silence_recently_spoken = False      # True after silence TTS completes
            session_data.last_tts_end_ts = 0.0                # Timestamp when TTS ended
            session_data.greeting_silence_allowed = True      # Allow silence during greeting
    ...
    async def _force_kill_silence_tts(self, session_id: str, session_data):
        """Immediately stop silence TTS on backend + instruct frontend to stop playing."""
        # Set hard cancel flag so the streaming loop also stops sending chunks
        session_data.cancel_silence_tts = True

        try:
            # Stop any active TTS stream for this session (not just by type)
            await self.tts_processor.stop_stream(session_id)
        except Exception as e:
            logger.warning(f"⚠️ TTS stop_stream failed: {e}")

        # Mark silence system as OFF
        session_data.silence_prompt_active = False
        session_data.silence_tts_active = False
        session_data.tts_active = False
        session_data.silence_followup_running = False
        session_data.awaiting_user = False

        # Send kill signal to frontend
        try:
            await session_data.websocket.send_text(json.dumps({
                "type": "kill_silence_prompt",
                "message": "stop"
            }))
            logger.info("🛑 Sent frontend kill signal for silence prompt")
        except Exception:
            logger.warning("⚠️ Failed to send silence kill signal to frontend")

   


        #to-kill-normal-prompt
    async def _kill_normal_tts(self, session_data):
        if session_data.normal_tts_active:
            session_data.normal_tts_active = False

            try:
                await self.tts_processor.stop_stream(session_data.session_id, stream_type="normal")
            except:
                pass

            logger.info("🛑 Normal AI TTS stopped cleanly")
    ...
    async def process_audio_with_silence_status(self, session_id: str, message_data: dict):
        ...
        try:
            # Extract status info
            raw_status = message_data.get('userStatus', 'unknown') or 'unknown'
            # ✅ SILENCE ENHANCEMENT: Normalize to one canonical form (handles "user_speaking", "userSpeaking", etc.)
            user_status = raw_status.lower().replace(" ", "_")
            is_user_speaking = user_status in ("user_speaking", "userspeaking")
            is_user_silent = user_status in ("user_silent", "usersilent", "silent")
            ...
            # ✅ SILENCE ENHANCEMENT: Keep status in session for TTS loop and silence logic
            session_data.is_user_speaking_live = is_user_speaking
            session_data.is_user_speaking = is_user_speaking
            session_data.last_status = user_status
            session_data.last_status_ts = time.time()

            # ✅ SILENCE ENHANCEMENT: Prevent next question if user interrupted AI
            if getattr(session_data, "last_status", None) == "user_interrupt":
                logger.info("🛑 AI audio interrupted by user — stopping next question generation")
                session_data.last_status = None
                return

            # ✅ SILENCE ENHANCEMENT: Hard cancel silence system if user speaks
            if is_user_speaking:
                # If user is speaking during NORMAL AI TTS => DO NOT CANCEL it
                if getattr(session_data, "normal_tts_active", False):
                    logger.info("🟢 User speaking over normal AI TTS — keeping prompt alive")
                    session_data.has_user_spoken = True
                    session_data.last_user_speech_ts = time.time()
                    return

                # Immediately kill silence prompt (backend + frontend)
                if getattr(session_data, "silence_tts_active", False):
                    logger.info("🛑 User speaking — forcing silence prompt cancellation")
                    await self._force_kill_silence_tts(session_id, session_data)

                # Hard reset silence counters
                session_data.consecutive_silence_chunks = 0
                #session_data.silence_response_count = 0
                session_data.has_user_spoken = True
                session_data.last_user_speech_ts = time.time()

                # Prevent new silence prompts for next 1.75 seconds
                session_data.silence_cooldown_until = time.time() + 1.75
                # Prevent immediate AI question after silence-kill
                session_data.block_next_ai_until_transcript = True
                
                # Additional resets
                session_data.silence_followup_running = False
                session_data.silence_followup_active = False
                session_data.silence_prompt_active = False
                session_data.awaiting_user = False

                logger.info("🔊 User resumed speaking — silence state fully reset")
                # DO NOT return here - continue with transcript path!
            ...
            is_silence_chunk = (
                ...
                and (silence_detected_flag or is_user_silent or user_status in ('user_silent', 'user_stopped_speaking'))
            )
            ...
            if user_status == 'user_speaking':
                ...
                session_data.has_user_spoken = True
                session_data.last_user_speech_ts = time.time()
                session_data.silence_followup_active = False  # ✅ SILENCE ENHANCEMENT
                session_data.awaiting_user = False  # ✅ SILENCE ENHANCEMENT
                session_data.silence_followup_running = False  # ✅ SILENCE ENHANCEMENT
                '''
            ...
            if is_silence_chunk:
                session_data.last_user_activity_ts = time.time()  # ✅ SILENCE ENHANCEMENT
                ...
                # ✅ SILENCE ENHANCEMENT: Global silence cooldown after user resumes speaking
                if hasattr(session_data, "silence_cooldown_until"):
                    if time.time() < session_data.silence_cooldown_until:
                        logger.info("⏸️ Silence cooldown active — skipping silence prompt")
                        return
                ...
                if session_data.consecutive_silence_chunks >= session_data.silence_chunks_threshold:
                    ...
                    # ✅ SILENCE ENHANCEMENT: Skip if AI is already responding
                    ...
                    # ✅ SILENCE ENHANCEMENT: Avoid rapid repetitive silence prompts
                    if getattr(session_data, "silence_response_count", 0) >= 3:
                        logger.info("🔇 Silence prompt threshold reached — slowing prompts, but session stays active.")
                        session_data.last_silence_prompt_ts = time.time()
                        # DO NOT return; allow user to resume at any point
                    
                    try:
                        # ✅ Use dynamic silence response generation with enhanced context
                        ctx = {
                            "name": session_data.student_name,
                            "silence_count": getattr(session_data, 'silence_response_count', 0) + 1,
                            "current_stage": session_data.current_stage.value,
                        ...
                        # ✅ SILENCE ENHANCEMENT: Clear recently_spoken flag after 1–2 seconds
                        #asyncio.create_task(self._clear_silence_recently_spoken(session_id))

                        soft_cutoff = getattr(session_data, "soft_cutoff_time", None)
                        if session_data.current_stage == SessionStage.TECHNICAL and (not soft_cutoff or now_ts < soft_cutoff):
                            session_data.awaiting_user = False  # ✅ Changed from True to match my_process_audio
                            session_data.last_user_activity_ts = time.time()
            ...
            # ✅ SILENCE ENHANCEMENT: Allow AI to continue only after we fully received a real transcript
            session_data.block_next_ai_until_transcript = False
            session_data.last_transcript = (transcript or "").strip()
            ...
            if transcript and transcript.strip():
                ...
                session_data.has_user_spoken = True
                session_data.last_user_speech_ts = time.time()
                session_data.clarification_attempts = 0  # ✅ SILENCE ENHANCEMENT: Reset clarification attempts

            # ✅ SILENCE ENHANCEMENT: Block AI question if transcript still forming after silence kill
            if getattr(session_data, "block_next_ai_until_transcript", False):
                logger.info("⛔ Blocking AI question — transcript still forming after silence kill.")
                return
    ...
    async def process_silence_notification(self, session_id: str, silence_data: dict):
        """
        Silence handler — fixed without removing any working logic.
        """
        ...
        # 🔥 NEW FIX: silence = activity (prevents idle)
        session_data.last_user_activity_ts = time.time()

        # 🧹 Keep your existing reset logic
        if session_data.silence_prompt_active and ( 
            time.time() - session_data.last_user_speech_ts) < 0.8 :
            logger.info("🔄 Silence prompt interrupted by user — resetting silence counter")
            session_data.silence_response_count = 0

        # 🚨 NEW FIX: kill silence mid-way if user speaks
        if silence_data.get("userSpeaking", False) or getattr(session_data, "is_user_speaking", False):
            logger.info("🛑 User speaking — terminate Silence system only")
            session_data.silence_prompt_active = False
            session_data.silence_followup_running = False
            session_data.awaiting_user = False
            session_data.cancel_silence_tts = True
            try:
                #await self.unified_tts.stop_stream(session_id)
                await self.tts_processor.stop_stream(session_id, stream_type="silence")
            except Exception as e:
                logger.warning(f"⚠️ Failed to stop silence TTS stream cleanly: {e}")
            return
        ...
        try:
            ...
            # 🔒 Keep your debounce logic
            last_prompt = getattr(session_data, "last_silence_prompt_ts", 0)
            if session_data.silence_prompt_active:
                #logger.info("🔁 Debounce skip")
                return

            # 🔒 Keep your TTS guard
            if getattr(session_data, "tts_active", False) or \
            (now_ts - getattr(session_data, "last_tts_end_ts", 0)) < 0.4:
                logger.info("🔇 TTS still finishing — skip silence")
                return

            # 🔒 Avoid duplicates — keep your guard
            if getattr(session_data, "silence_prompt_active", False):
                logger.info("🔒 Silence prompt already active — skip")
                return

            # 🔒 Avoid starting two followup loops
            if getattr(session_data, "silence_followup_running", False):
                # NEW: recover if follow-up has been running too long WITHOUT firing
                if (now_ts - getattr(session_data, "last_silence_prompt_ts", 0)) > 20:
                    logger.info("⏳ Follow-up loop stale — resetting silence system")
                    session_data.silence_followup_running = False
                else:
                    logger.info("🔁 Follow-up loop running — skip standalone silence")
                    return

            # 🔒 Greeting not finished — keep your original check
            if not getattr(session_data, "silence_ready", False):
                return

            # 🔒 Recording-active logic unchanged
            if silence_data.get("recordingActive"):
                if silence_data.get("userSpeaking", False):
                    # block ONLY if user actually spoke recently (<1 sec)
                    if (time.time() - session_data.last_user_speech_ts) < 2.0:
                        return

            # 🔒 Speech cooldown unchanged
            last_speech_ts = getattr(session_data, "last_user_speech_ts", 0)
            #cooldown_s = 1.5 if session_data.current_stage == SessionStage.GREETING else 2.0
            #if last_speech_ts and (now_ts - last_speech_ts) < cooldown_s:
                #return

            # 🔒 Grace window unchanged
            past_greeting_grace = (
            ...
            if not (session_data.has_user_spoken or past_greeting_grace):
                return
            
            # --- NEW: block short silences (< 4s)
            silence_duration = silence_data.get("silenceDuration", 0)
            if silence_duration < 5000:
                logger.info("⏸️ Silence too short (<5s) — skip")
                return

            # 🔒 Your silence counter
            session_data.consecutive_silence_chunks += 1
            if session_data.consecutive_silence_chunks < 1:
                return
            session_data.consecutive_silence_chunks = 0

            # 🔒 Keep your max prompt limit
            # 🔒 Enforce per-session max silence responses
            max_allowed = getattr(session_data, "max_silence_responses", 5)
            if session_data.silence_response_count >= max_allowed:
                session_data.last_silence_prompt_ts = time.time()
                logger.info(
                    "🔕 Max silence responses reached (%d/%d) — skipping new silence prompt",
                    session_data.silence_response_count, max_allowed,
                )
                return


            # ------------------------------
            # 🔥 NEW FIX: STOP OLD TTS BEFORE NEW ONE
            # ------------------------------
            try:
                #await self.unified_tts.stop_stream(session_id)
                await self.tts_processor.stop_stream(session_id)
            ...
            # --- NEW: If user suddenly speaks before silence prompt → cancel
            if (time.time() - getattr(session_data, "last_user_speech_ts", 0)) < 1.5:
                logger.info("🛑 User resumed speaking mid-silence → cancel silence prompt")
                session_data.silence_prompt_active = False
                return


            # 🔒 Your context construction preserved
            ctx = {
                "name": session_data.student_name,
                "silence_count": session_data.silence_response_count + 1,
                "current_stage": session_data.current_stage.value,
                "conversation_length": len(session_data.conversation_log),
                "time_elapsed": now_ts - session_data.created_at,
                "domain": getattr(session_data, "current_domain", "your work"),
                "last_topic": getattr(session_data, "current_concept", None),
                "silence_context": {
                    "notification_type": "standalone",
                    "timestamp": now_ts,
                    "phase": session_data.current_stage.value,
                },
            }

            # 🔒 Your LLM generation logic preserved
            text = await self.generate_dynamic_silence_response(session_data, ctx)
            if not text or len(text.split()) < 3:
                text = f"Take your time, {session_data.student_name}. We can continue whenever you're ready."

            # 🔒 Your conversation logging preserved
            #session_data.silence_response_count += 1
            session_data.add_exchange(text, "[USER_SILENT]", 0.0, "silence_handling", False)

            # 🔥 Mark prompt active (preserve your logic)
            ...
            # 🔥 Silence prompt ended
            #session_data.silence_prompt_active = False

            # 🔥 Start follow-up loop only once
            if not session_data.silence_followup_running:
                session_data.silence_followup_running = True
                asyncio.create_task(self._loop_silence_followup(session_data))

            # 🔒 Your awaiting_user logic preserved
            session_data.awaiting_user = True
    ...
    # =============================================
    # 🧩 Phase 1 Stable Version – Non-Blocking Cleanup
    # =============================================
    async def _send_silence_response_with_audio(self, session_data: SessionData, text: str):
        """
        Send silence response with proper TTS playback.
        Counter only increments if prompt completes WITHOUT user interruption.
        """
        now = time.time()

        # 🧠 Wait briefly if previous TTS just ended
        if (now - getattr(session_data, "last_tts_end_ts", 0)) < 1.2:
            wait_s = 1.2 - (now - session_data.last_tts_end_ts)
            logger.info("⏸️ Waiting %.2fs for previous TTS playback to finish", wait_s)
            await asyncio.sleep(wait_s)

        # 🆕 Auto-unlock stale silence lock
        if getattr(session_data, "silence_prompt_active", False):
            if (time.time() - getattr(session_data, "silence_prompt_active_ts", 0)) > 5:
                logger.warning("⚠️ Silence prompt lock stale — auto-reset")
                session_data.silence_prompt_active = False
            else:
                return

        # 🛑 EARLY INTERRUPT BEFORE STREAM STARTS (critical)
        if getattr(session_data, "last_status", "") == "user_speaking" or \
        (time.time() - getattr(session_data, "last_user_speech_ts", 0)) < 0.6:
            logger.info("🛑 EARLY INTERRUPT BEFORE STREAM STARTED — aborting silence prompt")
            session_data.silence_prompt_active = False
            return
        ...
        try:
            ...
            # 🔒 Lock playback flags
            session_data.silence_followup_running = False
            session_data.silence_tts_active = True
            session_data.normal_tts_active = False
            session_data.silence_prompt_active = True
            session_data.silence_prompt_active_ts = time.time()
            session_data.tts_active = True
            session_data.cancel_silence_tts = False

            # Set pending index for safety, same as refined version
            completed_count = getattr(session_data, "silence_response_count", 0)
            session_data.pending_silence_index = completed_count + 1
            ...
            try:
                ...
                logger.warning("⚠️ TTS restart error: %s", restart_err)
            ...
            async for audio_chunk in self.tts_processor.generate_ultra_fast_stream(
                ...
                # ========== INTERRUPT PATHS ==========
                if session_data.cancel_silence_tts:
                    interrupted = True
                    session_data.silence_prompt_active = False
                    break

                if getattr(session_data, "last_status", "") == "user_speaking":
                    interrupted = True
                    session_data.silence_prompt_active = False
                    break
                ...
                if (time.time() - getattr(session_data, "last_user_speech_ts", 0)) < 1.0:
                    interrupted = True
                    session_data.silence_prompt_active = False
                    break

                if getattr(session_data, "last_status", "") in ("user_speaking", "userspeaking"):
                    interrupted = True
                    session_data.silence_prompt_active = False
                    break
                ...
                if (time.time() - getattr(session_data, "last_user_speech_ts", 0)) < 1.2:
                    interrupted = True
                    session_data.silence_prompt_active = False
                    break
                ...
                if audio_chunk:
                    ...
                    try:
                        ...
                        logger.warning("⚠️ Chunk send failed: %s", send_err)
                        continue
            ...
            logger.info("✅ Streamed %d silence chunks (%s)", chunk_count, session_data.session_id)

        except Exception as e:
            logger.error("Silence TTS streaming error: %s", e)
            interrupted = True

        finally:
            # ==================================================================
            # ⭐ SIMPLIFIED COUNTER BLOCK (your version, merged cleanly)
            # ==================================================================

            if not interrupted and chunk_sent:
                session_data.silence_response_count += 1
                logger.info(
                    "✅ Silence prompt COMPLETED — counter now: %d/%d",
                    session_data.silence_response_count,
                    getattr(session_data, "max_silence_responses", 5)
                )

            elif interrupted:
                logger.info(
                    "↩️ Silence prompt INTERRUPTED — counter unchanged: %d/%d",
                    session_data.silence_response_count,
                    getattr(session_data, "max_silence_responses", 5)
                )

            elif not chunk_sent:
                logger.warning("❗ No silence audio chunks sent — counter unchanged")
            ...
            session_data.pending_silence_index = None  # discard safely

            session_data.block_next_ai_until_transcript = True
            session_data.silence_recently_spoken = True
            asyncio.create_task(self._clear_silence_recent_flag(session_data))
            asyncio.create_task(self._delayed_flag_cleanup(session_data))

            logger.debug("🧹 Silence-TTS cleanup complete (%s)", session_data.session_id)


    #helper-block
    #async def _cooldown_silence(self, session_id):
        #await asyncio.sleep(1.5)
        #s = self.active_sessions.get(session_id)
        #if s:
           # s.silence_recently_spoken = False

    
    #new-block
    async def _delayed_flag_cleanup(self, session_data: SessionData):
        """Asynchronous post-cleanup to release silence locks after short buffer."""
        await asyncio.sleep(0.5)
        try:
            session_data.silence_prompt_active = False
            logger.debug("🔓 Async silence prompt lock fully released for %s", session_data.session_id)
        except Exception as e:
            logger.warning("⚠️ Delayed cleanup failed for %s: %s", session_data.session_id, e)
    
    #another-new-block
    #async def _clear_silence_recent_flag(self, session_data):
        #await asyncio.sleep(1.0)
        #session_data.silence_recently_spoken = False
   
       #===============new function to generate silence prompt=======
    async def _loop_silence_followup(self, session_data: SessionData):
        """
        Periodic silence follow-up system:
        - Fires gentle silence prompts every 25–40 seconds
        - Stops instantly if user speaks OR AI is speaking
        - Never overlaps with questions or silence prompts
        - Runs entire session (not one-time)
        """
        try:
            logger.info("🔁 Silence follow-up loop started for session: %s", session_data.session_id)

            while session_data.is_active and session_data.silence_followup_running:

                # 🔥 HARD STOP — user spoke recently
                if (time.time() - getattr(session_data, "last_user_speech_ts", 0)) < 2.0:
                    logger.info("🛑 User spoke recently — skipping this follow-up cycle.")
                ...
                # 🔥 HARD STOP — user started speaking during wait
                if (now - getattr(session_data, "last_user_speech_ts", 0)) < 2.0:
                    logger.info("🛑 User spoke during delay — skipping follow-up.")
                    continue

                # Stop if AI question TTS is active
                if getattr(session_data, "tts_active", False):
                    logger.info("🔇 AI TTS active — pausing follow-up.")
                    continue

                # Stop if a silence prompt already playing
                if getattr(session_data, "silence_prompt_active", False):
                    logger.info("🔇 Silence prompt already active — skipping.")
                    continue

                # Final safety check
                if (time.time() - getattr(session_data, "last_user_speech_ts", 0)) < 2.0:
                    logger.info("🛑 User spoke — aborting follow-up attempt.")
                    continue

                # Build gentle prompt
                variants = [
                    f"Take your time {session_data.student_name}, I’m right here.",
                    f"No worries {session_data.student_name}, speak whenever you feel ready.",
                    f"I'm here when you’re ready to continue, {session_data.student_name}.",
                    f"Whenever you wish to continue, I'm here {session_data.student_name}.",
                    f"I'll wait patiently, {session_data.student_name}. Take your time.",
                ]
                gentle_line = random.choice(variants)

                await self._send_silence_response_with_audio(session_data, gentle_line)
                logger.info("🔊 Follow-up silence prompt sent.")

        except Exception as e:
            logger.error("❌ Error in silence follow-up loop: %s", e)

        finally:
            logger.info("🔁 Silence follow-up loop exited cleanly.")
    ...
    async def _send_response_with_ultra_fast_audio(self, session_data: SessionData, text: str):
       ...
       # -------------------------------------------
    # FIX: Missing silence reset helper
    # -------------------------------------------
    async def _clear_silence_recent_flag(self, session_data):
        """Reset silence flags after a silence TTS completes."""
        try:
            session_data.silence_prompt_active = False
            session_data.silence_recently_spoken = False
            #session_data.silence_followup_running = False
            ...
            session_data.last_user_activity_ts = time.time()
            logger.info("🔄 Silence flags reset (post-silence TTS)")
        except Exception as e:
            logger.error(f"Failed to clear silence flags: {e}")
